...
class MyLocalGatedGCN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        if x.dtype == torch.long:
            ...
            print(
                f"Warning: Unexpected node feature type {x.dtype} in model. Attempting to cast to long for nn.Embedding.")
        ...
        if hasattr(edge_attr, 'numel') and edge_attr.numel() > 0:
            if edge_attr.size(0) > 0:
                ...
                if edge_attr.shape[1] == expected_edge_dim_from_encoder:
                    ...
                    print(
                        f"Warning: Edge feature dim mismatch in model. Expected {expected_edge_dim_from_encoder}, got {edge_attr.shape[1]}. Creating zero edge features.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_rwse', action='store_false', dest='use_rwse_pe_cli',
                        help="Disable RWSE Positional Encoding.")  # Default is True if not specified
    ...
    # Possibility to choose loss

    cli_args = parser.parse_args()
    LOSS = cli_args.loss_fn
    ...
    print(
        f"Model: Layers={GNN_LAYERS}, HiddenDim={GNN_HIDDEN_DIM}, NodeEmb={NODE_EMBEDDING_DIM}, EdgeEmb={EDGE_EMBEDDING_DIM}")
    ...
    train_graphs = all_loaded_splits[DATASET]['train']
    val_graphs = all_loaded_splits[DATASET]['val']
    if not train_graphs or not val_graphs:
        print(f"No training data found for dataset {DATASET}. Exiting.")
        exit()

    test_datasets = {}
    for ds_name_iter in ['A', 'B', 'C', 'D']:  # Corrected iteration variable name
        test_datasets[ds_name_iter] = all_loaded_splits[ds_name_iter]['test']
    ...
    if train_graphs and hasattr(train_graphs[0], 'edge_attr') and train_graphs[0].edge_attr is not None and \
            train_graphs[0].edge_attr.numel() > 0:
        actual_edge_dim = train_graphs[0].edge_attr.shape[1]
        if actual_edge_dim != EDGE_FEATURE_DIM:
            print(
                f"WARNING: Loaded training data has edge_attr dim {actual_edge_dim}, but model expects {EDGE_FEATURE_DIM}. Ensure consistency from data_loader.py or update EDGE_FEATURE_DIM.")
    elif EDGE_FEATURE_DIM > 0:  # If we expect edge features but first graph has none/empty
        print(
            f"WARNING: Model expects edge_attr dim {EDGE_FEATURE_DIM}, but first training graph has no/empty edge_attr. This might be an issue if other graphs have edge_attr.")
    ...
    for epoch_iter in range(1, EPOCHS + 1):
        ...
        print(
            f"Epoch {epoch_iter:02d}/{EPOCHS:02d} | TrainLoss: {train_loss:.4f} | ValLoss: {val_loss:.4f} | ValAcc: {val_acc:.4f} | LR: {current_lr_val:.1e} | Time: {(time.time() - start_time):.2f}s")
    ...
    for ds_name, current_test_graphs in test_datasets.items():  # ds_name was ds_name_iter
        if current_test_graphs:
            ...
            test_predictions_array = eval_epoch(model, current_test_loader, criterion, device,
            ...
            if num_test_samples > 0:
                ...
                print(
                    f"  No test predictions generated for {ds_name} (dataset might be empty or predictions array is empty).")
        else:
